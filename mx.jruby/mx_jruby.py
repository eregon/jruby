...
class MavenBuildTask(mx.BuildTask):
    ...
    def build(self):
        ...
        mx.run(['./mvnw', '--version'], nonZeroIsFatal=False, cwd=rubyDir)

        mx.log('Building without tests')

        mx.run(['./mvnw', '-DskipTests', '-X', '-Dtruffle.version=' + truffle_commit], cwd=rubyDir)

        mx.log('Building complete version')

        mx.run(['./mvnw', '-Pcomplete', '-X', '-DskipTests', '-Dtruffle.version=' + truffle_commit], cwd=rubyDir)
...
#        shutil.rmtree(os.path.join(_suite.dir, 'lib', 'jni'), True)
    ...
    def clean(self, forBuild=False):
        if forBuild:
            return
        rubyDir = _suite.dir
        mx.run(['./mvnw', 'clean'], nonZeroIsFatal=False, cwd=rubyDir)
